...
@router.get("/get")
def get_memories(session_id: str = Depends(get_current_session_id)):
...
@router.post("/modify")
def modify_memory(req: MemoryModifyRequest, session_id: str = Depends(get_current_session_id)):
    """用户发送自然语言指令让 AI 修改记忆"""
    ...
    try:
        save_undo_snapshot(session_id)  # 修改前备份

        current = db_get(session_id) or {}
        ...
        upsert_memories(session_id, updated)
...
@router.post("/undo")
def undo_memory(session_id: str = Depends(get_current_session_id)):
    """撤销上次手动修改"""
...
def re_extract_memories(session_id: str = Depends(get_current_session_id)):
    """从全部对话历史重新提取记忆"""
...
@router.post("/clear")
def clear_memories(session_id: str = Depends(get_current_session_id)):
