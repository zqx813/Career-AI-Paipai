...
def send_message(req: ChatRequest):
    ...
    history = get_conversation_history(req.session_id, req.scenario, req.thread_id or '')

    # 自动创建线程
    thread_id = req.thread_id
    if not thread_id:
        thread_id = create_thread(req.session_id, req.scenario)
    else:
        existing_threads = [t['thread_id'] for t in
                            get_threads(req.session_id, req.scenario)]
        if not existing_threads:
            thread_id = create_thread(req.session_id, req.scenario, thread_id=thread_id)

    # 保存用户消息
    save_message(req.session_id, req.scenario, "user", req.message, thread_id)
    ...
    def generate():
        ...
        try:
            ...
            # 以下为后台操作，不阻塞用户界面
            save_message(req.session_id, req.scenario, "assistant", full_response, thread_id)
            ...
            if should_extract_memories(req.session_id):
                try:
                    _, since_id, max_id = get_new_message_stats(req.session_id)
                    new_msgs = get_messages_since(req.session_id, since_id)
                    ctx_msgs = get_thread_context(req.session_id, since_id, new_msgs)
                    existing = get_memories(req.session_id)
                    extracted = extract_memories(new_msgs, existing, ctx_msgs)
                    if extracted and any(v for v in extracted.values() if v):
                        upsert_memories(req.session_id, extracted, max_id)
                except Exception as e:
                    print(f"常规对话记忆提取失败: {e}")

            user_msgs = get_conversation_history(req.session_id, req.scenario, thread_id)
            if len([m for m in user_msgs if m['role'] == 'user']) == 1:
                existing = get_thread_titles(req.session_id, req.scenario)
...
@router.get("/history")
def get_history(session_id: str, scenario: str, thread_id: str = ""):
    """获取对话历史"""
...
@router.get("/threads")
def get_threads(session_id: str, scenario: str):
    """获取对话线程列表"""
